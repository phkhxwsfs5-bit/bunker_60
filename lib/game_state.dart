...
class GameState extends ChangeNotifier {
  ...
  // Canlýya Çýkarken Kullanýlacak GERÇEK AdMob ID'leri
  ...
  final Map<String, List<Map<String, String>>> _allTraits = {
    "Baba": [
      {"name": "Tamirci", "desc": "Alet kullanýmlarýnda %30 ihtimalle alet kýrýlmaz."},
      {"name": "Demir Mide", "desc": "Açlýða karþý daha dirençlidir, yavaþ acýkýr."},
      {"name": "Koruyucu", "desc": "Keþiflerde daha fazla eþya bulur ama yaralanma riski artar."},
      {"name": "Ýri Yarý", "desc": "Morali çok zor düþer ama daha çabuk susar."},
      {"name": "Soðukkanlý", "desc": "Gece yaþanan kötü olaylardan (hýrsýz, ses) etkilenmez."},
    ],
    "Anne": [
      {"name": "Pratik", "desc": "Dýþarýdaki keþif görevlerinden 1 gün erken döner."},
      {"name": "Tutumlu", "desc": "Birine çorba içirdiðinde %25 ihtimalle çorba eksilmez."},
      {"name": "Þifacý", "desc": "Bazen hastalandýðýnda ilaçsýz kendiliðinden iyileþebilir."},
      {"name": "Gözlemci", "desc": "Radyo aramalarýnda askeri frekans bulma þansý daha yüksektir."},
      {"name": "Dirençli", "desc": "Susuzluða karþý olaðanüstü dayanýklýdýr."},
    ],
    "Çocuk": [
      {"name": "Çevik", "desc": "Keþif görevlerinde asla yaralanmaz."},
      {"name": "Neþeli", "desc": "Onunla sohbet etmek tüm ailenin moralini artýrýr."},
      {"name": "Ufaklýk", "desc": "Su ve çorbayý çok daha yavaþ tüketir."},
      {"name": "Þanslý", "desc": "Sýðýnaða saldýran böcek, hýrsýz gibi belalarý kazara savuþturabilir."},
      {"name": "Gözü Açýk", "desc": "Keþiflerde mutlaka gizli bir ekstra eþya bulur."},
    ]
  };
  ...
  Future<void> nextDay() async {
    ...
    if (characters.every((c) => !c.isAlive)) {
      isGameOver = true;
      currentEvent = GameEvent(description: "Herkes öldü... Sýðýnak artýk sessiz bir mezar.", choices: []);
      isDayChanging = false;
      notifyListeners();
      deleteSaveData();
      return;
      ...
      if (daysUntilReturn <= 0) {
        ...
        if (Random().nextInt(100) < 20) {
          ...
          currentEvent = GameEvent(
            description: "${currentEvent.description}\n\n$exploringCharacter hala dýþarýda, dönmesini bekliyoruz.",
    ...
    if (diedTonight.isNotEmpty) {
      String deathMsg = "KÖTÜ HABER! Bu sabah uyandýðýmýzda ${diedTonight.join(' ve ')} nefes almýyordu. Açlýk ve susuzluða daha fazla dayanamadý...\n\n";
      currentEvent = GameEvent(
  ...
  void _generateExpeditionEvent() {
    ...
    if (rnd == 0) {
      currentEvent = GameEvent(
        description: "TELSÝZ: $exploringCharacter terk edilmiþ bir dükkanda kilitli bir çelik kasa buldu. Açmayý denesin mi?",
        choices: [
          EventChoice(
            ...
            onSelect: () {
              ...
              currentEvent = GameEvent(description: "Kasa açýldý! Ýçinden 2 Su ve 2 Çorba çýktý. $exploringCharacter yola devam ediyor.", choices: []);
              notifyListeners(); saveGame();
            }
          ),
          EventChoice(
            buttonText: "Sessizce Býrak",
            isEnabled: true,
            onSelect: () {
              currentEvent = GameEvent(description: "Riski göze almadýk. $exploringCharacter sessizce keþfe devam ediyor.", choices: []);
              notifyListeners(); saveGame();
            }
          )
        ]
      ...
      currentEvent = GameEvent(
        description: "TELSÝZ: $exploringCharacter yolda yaralý, çaresiz bir yabancýya rastladý. Telsizden 'Ona yardým edeyim mi?' diye soruyor.",
        choices: [
          EventChoice(
            ...
            onSelect: () {
              ...
              if (!isUtopiaRevealed) {
                ...
                currentEvent = GameEvent(description: "Yabancý suyu içti ve minnettarlýkla bize kendi kendine yetebilen efsanevi 'Ütopya' kolonisinin koordinatlarýný verdi! Haritaya eklendi.", choices: []);
              } else {
                medkitCount++;
                currentEvent = GameEvent(description: "Yabancý minnettarlýkla çantasýndaki son Ýlk Yardým Kitini bize verdi.", choices: []);
              }
              notifyListeners(); saveGame();
            }
          ),
          EventChoice(
            ...
            isEnabled: true,
            onSelect: () {
              currentEvent = GameEvent(description: "Acýmasýz bir dünyadayýz. Yabancýyý ölüme terk edip yola devam ettik. (Herkesin morali düþtü)", choices: []);
      ...
      currentEvent = GameEvent(
        description: "TELSÝZ: $exploringCharacter dost canlýsý görünen bir hayatta kalanla karþýlaþtý. 2 Çorba karþýlýðýnda 1 Ýlk Yardým Kiti takasý teklif ediyor.",
        choices: [
          EventChoice(
            ...
            onSelect: () {
              soupCount -= 2;
              medkitCount++;
              currentEvent = GameEvent(description: "Takas baþarýlý! Ýlk Yardým Kiti envantere eklendi.", choices: []);
              notifyListeners(); saveGame();
            }
          ),
          EventChoice(
  ...
  void _applyEndOfDayTraits(Character char) {
    ...
    if (trait == "Ýri Yarý" && rand.nextInt(100) < 30) char.thirstLevel++;
    if (trait == "Dirençli" && char.thirstLevel > 0 && rand.nextInt(100) < 15) char.thirstLevel--;
    if (trait == "Þifacý" && char.status == 'sick' && rand.nextInt(100) < 10) char.status = 'normal';
  ...
  void _loadEventForDay() {
    if (currentDay == 1) {
      currentEvent = GameEvent(description: "Sýðýnaða ulaþtýk. Þimdilik güvendeyiz. Bombanýn sesi kulaklarýmýzý saðýr edecek gibiydi. Dýþarý çýkamayýz.", choices: []);
    } else if (currentDay == 2) {
      triggerEffects(flicker: true, shake: true);
      currentEvent = GameEvent(description: "Ýlk gece çok zordu. Yukarýdan garip týkýrtýlar duyduk. Ampul sürekli göz kýrpýyor.", choices: []);
    } else {
  ...
  void useRadio(String action) {
    ...
    if (action == "scan") {
      ...
      if (currentDay > 15 && !isMilitaryEvacRevealed && chance < 15) {
        ...
        _injectSpecialPlaces();
        currentEvent = GameEvent(description: "RADYODA NET BÝR SÝNYAL! 'Hayatta kalanlar, yakýnlardaki Askeri Tahliye Noktasýna gelin. Kurtarma ekipleri hazýr.' Haritaya eklendi!", choices: []);
        notifyListeners(); saveGame(); return;
      }

      if (chance < 30) {
        currentEvent = GameEvent(description: "Radyodan parazitli bir ses geliyor... 'Orada kimse var mý? Lütfen yardým edin...' Ses aniden kesildi.", choices: []);
      } else if (chance < 60) {
        currentEvent = GameEvent(description: "Askeri bir frekans yakaladýk! 'Sektör 4 temiz. Beklemede kalýn.' Bizim için umut olabilir.", choices: []);
      } else if (chance < 80) {
        currentEvent = GameEvent(description: "Sadece parazit ve statik elektrik sesi var. Dýþarýda kimse kalmamýþ gibi...", choices: []);
      } else {
        currentEvent = GameEvent(description: "Garip bir müzik kanalý bulduk. Çalan þarký savaþ öncesi günleri hatýrlattý. Herkesin morali biraz olsun yerine geldi.", choices: []);
        for (var c in characters) {
          if (c.isAlive && !c.isExploring && c.moraleLevel > 0) c.moraleLevel -= 1;
        }
      }
    } else if (action == "music") {
      currentEvent = GameEvent(description: "Radyoyu sadece müzik dinlemek için açtýk. Klasik müzik sýðýnaðýn soðuk duvarlarýnda yankýlandý. Moraller düzeldi.", choices: []);
      for (var c in characters) {
        if (c.isAlive && !c.isExploring && c.moraleLevel > 0) c.moraleLevel -= 1;
      }
    }
  ...
  void triggerSalvation() {
      isGameWon = true;
      currentEvent = GameEvent(description: "Dýþarýdan kapý zorlandý! Askerler içeri girdi. Baþardýk, kurtulduk!", choices: []);
      notifyListeners();
      deleteSaveData();
  }
  ...
  GameEvent _getRandomEvent() {
    ...
    if (eventId == 99) {
        ...
        return GameEvent(
            description: "Dýþarýdan sesler geliyor! Kapaða vuruyorlar. 'Ordu Birlikleri, açýn!' Kurtulduk mu?",
            choices: [EventChoice(buttonText: "Kapaðý Aç", isEnabled: true, onSelect: () => triggerSalvation())]
        );
    } else if (eventId == 1) {
      triggerEffects(flicker: true);
      AudioManager().playSFX('event_bugs.mp3'); 
      return GameEvent(
        description: "Devasa, parlayan mutant hamamböcekleri çorba kutularýmýza saldýrýyor!",
        choices: [
          EventChoice(
            ...
            onSelect: () {
              if (toolCount > 0) {
                ...
                if (breakTool) toolCount--; 
                currentEvent = GameEvent(description: "Hepsini ezerek öldürdük. Eþyalarýmýz güvende.", choices: []);
              }
              notifyListeners();
              saveGame(); 
            },
          ...
          EventChoice(
            buttonText: "Saklan",
            onSelect: () {
              if (getTraitName("Çocuk") == "Þanslý" && characters[2].isAlive && !characters[2].isExploring && Random().nextBool()) {
                 currentEvent = GameEvent(description: "Böcekler konserveleri açamayýp kendi kendilerine gittiler! Þanslýyýz.", choices: []);
              } else {
                 soupCount = max(0, soupCount - 2);
                 currentEvent = GameEvent(description: "Böcekler 2 çorbamýzý yiyip uzaklaþtý...", choices: []);
              }
              notifyListeners();
              saveGame(); 
            },
      ...
      return GameEvent(
        description: "Sýðýnaðýn havalandýrmasý bozuldu. Ýçeriye zehirli hava sýzýyor.",
        choices: [
          EventChoice(
            ...
            onSelect: () {
              if (toolCount > 0) {
                toolCount--; 
                currentEvent = GameEvent(description: "Zor oldu ama tamir ettik. Ciðerlerimiz bayram etti.", choices: []);
              }
              notifyListeners();
              saveGame(); 
            },
          ...
          EventChoice(
            ...
            onSelect: () {
              _makeEveryoneSick();
              currentEvent = GameEvent(description: "Hava kalitesi çok kötü, herkes hastalandý.", choices: []);
              notifyListeners();
              saveGame(); 
            },
          ),
      ...
      AudioManager().playSFX('event_door_bang.mp3'); 
      return GameEvent(
        description: "GÜM! GÜM! Gece vakti sýðýnak kapaðý yumruklanýyor. Seslerden anladýðýmýz kadarýyla yaðmacýlar!",
        choices: [
          EventChoice(
            buttonText: "Silahla Korkut (Cephane)", isEnabled: ammoCount > 0,
            onSelect: () {
              if(ammoCount > 0) ammoCount--;
              currentEvent = GameEvent(description: "Havaya bir el ateþ ettik. Adým sesleri koþarak uzaklaþtý.", choices: []);
              notifyListeners();
              saveGame(); 
            }
          ),
          EventChoice(
            ...
            onSelect: () {
              ...
              soupCount = max(0, soupCount - 2);
              currentEvent = GameEvent(description: "Kapaðý kýrýp içeri daldýlar! Erzaklarýmýzýn bir kýsmýný (2 Su, 2 Çorba) çalýp gittiler.", choices: []);
              notifyListeners();
              saveGame(); 
            }
          )
      ...
      return GameEvent(
        description: "Duvardaki eski borulardan þiddetli bir týslama sesi geliyor. Sýðýnaða boðucu bir gaz sýzýyor!",
        choices: [
          EventChoice(
            buttonText: "Bezlerle Týka", isEnabled: true,
            onSelect: () {
              _makeSomeoneTiredOrSick();
              currentEvent = GameEvent(description: "Bütün gece zehirli gazý soluyarak borudaki çatlaðý bezlerle týkamaya çalýþtýk. Sýzýntý durdu ama çok yorulduk, birimiz hastalandý.", choices: []);
              notifyListeners();
              saveGame(); 
            }
          ),
      ...
      return GameEvent(
        description: "Köþedeki kutularýn arkasýnda kapaðý þiþkin ve etiketi silinmiþ iki konserve bulduk. Erzaklarýmýza ekleyelim mi?",
        choices: [
          EventChoice(
            buttonText: "Stoða Ekle", isEnabled: true,
            onSelect: () {
               if (Random().nextBool()) {
                 soupCount += 2;
                 currentEvent = GameEvent(description: "Konserveler hala saðlammýþ! Gýdamýz arttý (+2 Çorba).", choices: []);
               } else {
                 _makeSomeoneTiredOrSick();
                 currentEvent = GameEvent(description: "Kötü fikirdi. Kutular delikmiþ, sýzan pis koku birimizi hasta etti.", choices: []);
               }
               notifyListeners();
               saveGame(); 
            }
          ...
          EventChoice(
            ...
            onSelect: () {
              currentEvent = GameEvent(description: "Riske girmeye deðmez. Çöpe attýk.", choices: []);
              notifyListeners();
              saveGame(); 
            }
          )
      ...
      return GameEvent(
        description: "Sýðýnaðýn karanlýðý ve sessizliði yavaþ yavaþ akýl saðlýðýmýzý etkiliyor. Biri köþede kendi kendine konuþuyor.",
        choices: [
          EventChoice(
            buttonText: "Uzun Uzun Sohbet Et", isEnabled: true,
            onSelect: () {
              currentEvent = GameEvent(description: "Eski günlerden, güzel anýlardan bahsettik. Zor oldu ama toparlandýk.", choices: []);
              for(var c in characters) {
      ...
      return GameEvent(
        description: "$currentDay. Gün. Sýðýnakta sessiz bir bekleyiþ sürüyor. Kaynaklarý idareli kullanmalýyýz.", 
        choices: []
      );
    }
  }
  ...
  void talkToCharacter(Character char) {
    if (!hasChattedToday && char.isAlive && (char.moraleLevel > 0 || char.status == 'insane')) {
      ...
      if (char.name == "Çocuk" && getTraitName("Çocuk") == "Neþeli") {
        for (var c in characters) {
           if (c.isAlive && !c.isExploring && c.moraleLevel > 0) c.moraleLevel--;
        }
      }
  ...
  void startExpedition(String characterName, Place destination) {
    ...
    currentEvent = GameEvent(description: "$characterName, ${destination.name} bölgesine yola çýktý. Umarým sað döner.", choices: []);
    notifyListeners();
    saveGame();
  }
  ...
  void _handleExpeditionReturn() {
    if (exploringDestinationType == 'military') {
      isGameWon = true;
      currentEvent = GameEvent(description: "Ýnanamýyorum! Kurtarma helikopteri sýðýnaðýn tam üzerine indi! $exploringCharacter yolu açtý. BAÞARDIK, KURTULDUK!", choices: []);
    ...
    if (exploringDestinationType == 'utopia') {
      if (waterCount >= 5 && soupCount >= 5) {
        isGameWon = true;
        currentEvent = GameEvent(description: "Erzaklarýmýzý sýrtlandýk ve $exploringCharacter rehberliðinde Gizli Yeraltý Kolonisine ulaþtýk. Burada yeni bir hayat baþlýyor. KAZANDINIZ!", choices: []);
        notifyListeners(); deleteSaveData(); return;
      } else {
        String failText = "HAYAL KIRIKLIÐI! $exploringCharacter koloniyi buldu ama içeri kabul edilmemiz için yanýmýzda en az 5 Su ve 5 Çorba götürmemiz gerektiðini söylediler. Erzak yetersizliðinden geri çevrildik.";
    ...
    bool isScavenger = (exploringCharacter == "Çocuk" && getTraitName("Çocuk") == "Gözü Açýk");

    if (chance < 20 && !isAgile) {
      resultText = "KÖTÜ HABER! $exploringCharacter, dönüþ yolunda yaðmacýlarýn pususuna düþtü. Yaralý döndü!";
      for (var char in characters) {
        if (char.name == exploringCharacter) char.status = 'injured';
      }
    } else if (chance < 40 && !isScavenger) {
      resultText = "$exploringCharacter sýðýnaða döndü. Gittiði bölge ($exploringDestinationName) yaðmalanmýþ, bir þey bulamadý.";
    } else {
      ...
      if (exploringDestinationType == 'pharmacy') {
        medkitCount += (1 + extra);
        waterCount += 1;
        resultText = "BAÞARI! $exploringCharacter eczaneden Ýlk Yardým Kiti ve Su buldu!";
        ...
        soupCount += foundSoup;
        resultText = "BAÞARI! $exploringCharacter marketinden $foundWater su ve $foundSoup çorba getirdi!";
      } else if (exploringDestinationType == 'hardware') {
        toolCount += (1 + extra);
        resultText = "BAÞARI! $exploringCharacter nalburdan Alet Çantasý buldu!";
      } else {
        int foundWater = Random().nextInt(2) + 1 + extra;
        int foundSoup = Random().nextInt(2) + 1 + extra;
        waterCount += foundWater;
        soupCount += foundSoup;
        resultText = "BAÞARI! $exploringCharacter yanýna $foundWater su ve $foundSoup çorba getirdi!";
      }
    }
  ...
  void _injectSpecialPlaces() {
    if (isMilitaryEvacRevealed && !nearbyPlaces.any((p) => p.type == 'military')) {
      nearbyPlaces.add(Place(name: "Askeri Tahliye Noktasý", lat: lat + 0.015, lng: lng - 0.015, type: 'military'));
    }
    if (isUtopiaRevealed && !nearbyPlaces.any((p) => p.type == 'utopia')) {
      nearbyPlaces.add(Place(name: "Gizli Yeraltý Kolonisi (Giriþ için 5 Su/5 Çorba)", lat: lat - 0.012, lng: lng + 0.012, type: 'utopia'));
    }
  }
  ...
  Future<void> fetchNearbyPlaces() async {
    ...
    try {
      ...
      if (cachedLat != null && cachedLng != null && cachedPlacesJson != null) {
        ...
        if (distance < 500) {
          loadingMessage = "Harita Hafýzadan Yükleniyor...";
          notifyListeners();
    ...
    } catch (e) { print("Önbellek okuma hatasý: $e"); }
    ...
    loadingMessage = "Sinyal Zayýf. Tahmini Harita Kullanýlýyor...";
    notifyListeners();
    ...
    try {
      ...
      List<String> pharmacyNames = ["Þifa Eczanesi", "Merkez Eczanesi", "Saðlýk Eczanesi", "Umut Eczanesi", "Hayat Eczanesi", "Güneþ Eczanesi", "Halk Eczanesi", "Yeni Eczane"];
      ...
      List<String> marketNames = ["Güven Süpermarket", "Büyük Gýda", "Kardeþler Bakkalý", "Ucuza Market", "Bereket Gýda", "Merkez Hipermarket", "Bizim Bakkal", "Köþe Market"];
      ...
      List<String> hardwareNames = ["Usta Nalburiye", "Yapý Market", "Çýnar Hýrdavat", "Emin Yapý", "Kardeþler Nalbur"];
